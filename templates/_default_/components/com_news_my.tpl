...
<div class="float_bar">
    <a href="/news/add.html" class="usr_article_add">{$LANG.ADD_ARTICLE}</a>
</div>
...
<table width="100%" cellpadding="8" cellspacing="0" border="0" class="art_list">
	...
	{foreach key=tid item=article from=$articles}
            <tr>
                ...
                <td align="center">
                    <a href="/news/edit{$article.id}.html" title="{$LANG.EDIT}"><img src="/templates/{template}/images/icons/edit.png" border="0"/></a>
                    {if $user_can_delete}
                            <a href="/news/delete{$article.id}.html" title="{$LANG.DELETE}"><img src="/templates/{template}/images/icons/delete.png" border="0"/></a>
                    {/if}
                </td>
		</tr>			
	{/foreach}
