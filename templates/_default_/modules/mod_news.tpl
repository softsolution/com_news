...
function newsPage(page, module_id){
    $.post('/modules/mod_news/ajax/latest.php', {'module_id': module_id, 'page':page}, function(data){
	...
	<div class="mod_latest_entry">
        {if $article.image}
            <div class="mod_latest_image">
                <img src="/images/news/small/{$article.image}" border="0" width="32" height="32" alt="{$article.title|escape:'html'}"/>
            </div>
        {/if}
...
{if $cfg.showrss}
    <div class="mod_latest_rss">
        <a href="/rss/news/all/feed.rss">{$LANG.LATEST_RSS}</a>
    </div>
{/if}
